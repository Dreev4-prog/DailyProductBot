...
## Версия 2.2 — выбор категории
- При добавлении и редактировании товара категория выбирается кнопками: 50–100 €, 100–200 €, 200–500 €.
- Пользователь получает по одному случайному товару за нажатие.
- Перед каждым товаром можно выбрать ту же или другую категорию.
- Лимит — 2 товара за календарный день; в 00:00 по TIMEZONE лимит обнуляется.
- Один и тот же товар повторно одному пользователю не выдаётся.
- Автоматическая раздача отключена: пользователь сам забирает товары.
